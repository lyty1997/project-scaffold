...
- 禁止臆测用户的想法，不得把用户未明确说明的意图、偏好、优先级、验收标准、业务事实或授权补成“合理假设”。
- 对能从仓库、文档或工具确认的事实先查证；查证后仍不确定，或事项本质上只能由用户决定时，必须明确写出已知事实、不确定项、可选方案及其影响，请用户决策。在用户确认前，不得实施依赖该决定的部分，也不得用“默认方案”“更保守方案”或“可否决的判断”代替确认。

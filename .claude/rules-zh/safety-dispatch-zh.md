...
[English](../rules/safety-dispatch.md) | 中文
...
| 技术栈 | 对应规范 |
|-------|---------|
| Python | `python-coding-rules-zh.md` |
| TypeScript/JS | `typescript-coding-rules-zh.md` |
| Rust | 待建 |
| Go | 待建 |
| 跨语言 — 并发/资源 | `concurrency-resource-safety-zh.md`（asyncio task / 子进程 / pipe drain / shutdown 顺序 / shell trap） |

## 扩展新技术栈
1. 在 `.claude/hooks/post-edit-safety.py` 的 `CHECKS` 和 `EXT_MAP` 中添加新语言的检查命令
2. 在 `.claude/rules/` 中创建英文编码规范，并在 `.claude/rules-zh/` 中创建同步中文译本
3. 同步更新两种语言的表格

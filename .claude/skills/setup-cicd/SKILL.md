...
先读设计真相源 [`docs/architecture/cicd-autosetup.md`](../../../docs/architecture/cicd-autosetup.md) 与行为约束 [`.claude/rules/cicd-workflow.md`](../../rules/cicd-workflow.md)。
...
3. **与用户拍板探测不出来的项**（这一步不能省）
   至少确认：构建命令、测试命令、部署目标、发布触发方式。命令要么从项目已声明的脚本里读，要么问用户，**不得发明**。用户此刻答不上来的，就先不生成对应 workflow，把它记进 `docs/architecture/open-decisions.md`，不要为了"看起来完整"硬编一个。
...
4. **写台账 `docs/contracts/cicd-answers.json`**
   字段至少包含 `workflows`（含 `id` / `file` / `kind` / `displayName` / `on` / `permissions` / `jobs`）、`targets`（每个都必须有 `rollback`）、`secrets`（每个都要写 `source`）。部署步骤打 `"deployStep": true`，渲染器会自动加上"PR 不发布 + 手动触发默认演练"的闸门。

5. **生成并过本地门禁**
   `npm run gen:cicd` → `npm run quality`。渲染器会因违反不变量而硬失败（未钉 SHA 的第三方 action、`pull_request_target`、`continue-on-error: true`、错误的 secrets 写法）——**这些报错要按提示改台账，不要去改渲染器或放宽扫描器**。
...
7. **按真绿判据断言**（见下节）。红了就取日志定位、修台账、重新生成、再推，直到全绿。修复过程中每次提交都要用本仓库的提交格式，否则 `commit-msg` hook 会直接拒绝、循环卡死。
...
9. **回写台账与文档**
   更新台账的变更记录、`docs/progress.md`；涉及部署方式变化时同步 `docs/`。本地提交，**不自动 push**，是否合并问用户。
...
`gh api` / `gh run` 调用要包 3 次重试加退避，并严格区分"API 调用失败（UNKNOWN）"和"检查结论为失败"——拿不到数据绝不默认放行。

## 常见坑点

- **`gh api` 对 403/404 一律 `exit 1`，错误 JSON 走 stdout**：判定要解析响应体的 `.status`，不能只看退出码。也不要用 `gh auth status` 判认证——它超时时仍返回 0。
- **`GITHUB_TOKEN` 推的提交与 tag 不触发下游 workflow**："CI 自动打 tag → tag 触发发布"这条链用默认 token 会静默断掉。要么打 tag 与发布放同一个 job，要么用 `workflow_run` 显式串联。
...
- **生成物不要手工编辑**：带 `managed-by` 标记的文件被改过，`check:cicd` 会报漂移。改台账再 `npm run gen:cicd`。

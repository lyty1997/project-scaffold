...
This project vendors a reviewed Archify commit and disables automatic update checks. Do not run `scripts/check-update.mjs` during diagram work. Upgrades require an explicit maintainer task that updates the pinned commit in `docs/architecture/diagram-system.md`, reviews upstream changes, reapplies `LOCAL_CHANGES.md`, and reruns the complete validation and visual-review loop.
...
For a Markdown PNG committed by this project, never copy or crop a `visual-check` screenshot. Run `npm run review:diagrams -- <source-file>` from the repository root; the project wrapper keeps screenshots in a temporary evidence directory, then invokes the trusted HTML Viewer’s native `Archify.exportMenu.run("png")` path and writes the canonical full-diagram PNG. Read the project-specific PNG section in `references/delivery-contract.md` when a committed `.archify.png` is in scope.
...
Return the checked HTML path, diagram type, validation summary, specification/artifact receipt, and truthful visual-review status. When a committed documentation PNG is in scope, also return its path, native pixel dimensions, export scale, and `canonical=true` status. Do not claim success for a non-zero command or claim visual inspection you did not perform.

...
## Project Markdown PNG delivery

This project keeps two bitmap outputs with different jobs:

- `visual-check.*.png` is a full-page Viewer screenshot used only as temporary perceptual evidence.
- `docs/diagrams/<name>.archify.png` is the committed Markdown image and must be the Viewer-native canonical full-diagram PNG.

Never copy, rename, crop, or post-process a `visual-check` screenshot into the committed PNG. From the repository root run:

```bash
npm run review:diagrams -- <name>.<type>.json
```

The wrapper first performs the normal `deliver` and `visual-check` flow. It then loads that exact trusted HTML in Chrome and calls `Archify.exportMenu.run("png")`, the same path as **Export → PNG**. It intercepts only the browser download transport so the generated Blob can be written to the repository path; it does not replace `serializeSvg`, rasterization, theme resolution, or canvas encoding.

Acceptance requires all of the following:

- Viewer receipt `format=png` and `canonical=true`;
- downloaded filename ends in `.png` and Blob bytes equal the written file bytes;
- PNG IHDR dimensions equal the canonical SVG viewBox multiplied by the Viewer’s safe integer raster scale in `{4,3,2,1}` under its 16-megapixel cap;
- the image contains the full diagram and no title bar, chapter rail, toolbar, export menu, semantic controls, or navigation dock;
- `npm run check:diagrams` passes after the export.

The temporary visual-check screenshots still need human inspection in both themes. Separately inspect the native PNG for completeness and export cleanliness before reporting success.
...
When this project commits a Markdown PNG, append:

```text
native_png: /absolute/path/to/<name>.archify.png
native_png_dimensions: <width>x<height>
native_png_scale: 1|2|3|4
native_png_canonical: true
```

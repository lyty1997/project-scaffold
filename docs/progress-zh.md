...
## 2026-09-07 CST / 面向 gpt-6-astra 精简 Agent 提示词

- 交付范围：用户要求提交并推送。本次使用当前 `dev` 分支，一并包含此前的英文指令清理和对应语言门禁；远端交付按推送提交的 CI 运行结果核验。
- 评估：梳理根指令、任务路由、Claude 规则与 hooks、第一方 Skills 和固定版本 Archify 桥接。在修改前将设计和 OpenAI 官方依据写入 [Agent 提示词设计](architecture/agent-prompts-zh.md)，保留已有未提交的英文指令清理和六项推理原则。
- 修改：集中共用约束，将重复 Claude 规则改为条件链接，精简 workflow 与 Skill 流程，移除不存在的命令、自动技术栈选择、虚构测试角色和冲突的审批、推送要求。同步 CI/CD 设计与技术栈配方，新增可复用的提示词漂移指导。
- 源码层消融：以任务开始快照为基线，逐层对比根入口/workflow、专用规则、第一方 Skills，使用相同的八个人工验收场景。第一方指令由 90,672 降到 36,538 UTF-8 字节（59.7%）；含未修改的 Archify 入口时由 107,105 降到 52,971（50.5%）。不将此前删除译本、设计和历史正文、已安装 Skills 或宿主上下文计入缩减收益。
- 验证：沙箱以 `EPERM` 拒绝 CI/CD fixture 嵌套进程后，`npm run quality` 在沙箱外全部通过。四个第一方 Skills 通过元数据校验。使用 `.jpg` 扩展名的合成 uint16 TIFF 验证了临时预览输出、1280×853 灰度转换和原文件字节不变。人工加载路径与约束复核保留八项预期结果，并修正了编辑译本时的语言阅读例外。
- 剩余不确定性：Astra 实际耗时、token 用量、审批频率和任务质量需通过受控任务回放验证；环境挂载遮蔽 `.agents` 桥接后的新会话发现行为仍需在环境层检查。本任务未新增或修改脚本、依赖、workflow YAML、UI、vendored 包、用户数据收集或第三方服务。

## 2026-09-07 CST / Agent 指令加入推理与设计原则

- 完成：在 `AGENTS.md` 中新增六条可执行原则：第一性原理、适时对抗式审查、自主消融实验与奥卡姆剃刀、显式报告不确定性、独立思考与批判性思维，以及持续检查高内聚低耦合。消融实验每次移除或禁用一个要素，以相同验收检查与基线比较，并保持可逆且处于已授权范围内。
- 范围：沿用指令仅保留英文的契约，并链接现有架构原则。本次只更新指令，没有可执行的设计或实现变更可供消融。对未来 Agent 行为的实际影响尚未验证，应在后续任务中评估。
- 验证：`npm run check:docs`、`npm run check:localization` 和 `git diff --check` 均通过；已对照现有决策边界和架构规范复核六条原则。
- 遗留：本次文档修改无遗留。未新增用户数据收集或第三方服务。

## 2026-09-06 CST / Agent 与贡献协作指令仅保留英文

- 范围：清理前先收窄本地化契约，持续维护的中文译本仅覆盖项目说明和设计文档，即 `README.md`、`SCAFFOLD.md` 和 `docs/` 下的 Markdown。本决策取代 2026-08-31 记录中的指令翻译范围；旧条目仍作为历史保留。
- 清理：删除 31 份中文指令和协作流程副本（根目录 Agent 与贡献入口、Codex 规则及已知问题、Claude 规则、第一方 Skills 和 PR 模板），移除对应语言切换链接，并将保留的中文项目文档链接改指规范英文指令。20 份中文项目文档全部保留。同步修正现行语言、技术栈分发、贡献和已知问题规则，取消维护中文指令的要求。
- 门禁：`check:localization` 仅要求项目文档成对互链，并拒绝中文指令副本，包括大写 `-ZH.md` 变体及非空翻译规则目录。默认英文扫描、功能性中文数据和 vendored 例外继续保留。
- 验证：现有 CI/CD fixture 在托管沙箱内启动嵌套 Node 时被 `EPERM` 阻止，放行后完整 `npm run quality` 通过。17 个临时本地化正反场景通过，覆盖保留配对、拒绝指令副本与翻译规则目录，以及现有例外。Markdown 链接、语言配对和 `git diff --check` 均通过；独立复核确认删除与保留数量准确。
- 遗留：无。未新增用户数据收集、遥测、第三方服务、依赖或网络集成。

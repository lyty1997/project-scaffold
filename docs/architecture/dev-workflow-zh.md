...
[English](dev-workflow.md) | 中文

状态：active
最近更新：__在实际使用时更新为当前日期__
适用范围：本地渲染端机器与远端托管机之间的本地开发预览闭环（编码会话可以在任意一端发起，托管固定在远端托管机一端）。**不决定生产部署目标**——GitHub Pages / Cloudflare Pages / Vercel / 自托管仍是 [待决策问题](open-decisions-zh.md) 中的未决项，本工作流只覆盖"改代码 → 本地渲染验证 → 再改"的迭代环节。
...
沿用全局 git 工作流约定（`main` 稳定 / `dev` 开发主干 / `feature/description` 英文特性分支），并在远端托管机新增一个专用于预览的 worktree，理由是：远端托管机端可能同时存在"CLI 自己在 `dev` 上直接改动"和"某个 `feature` 分支需要马上预览"两件事，两者不应该共用同一个工作目录互相打扰。
...
- 本地渲染端：clone 同一个仓库（`https://github.com/__GITHUB_OWNER__/__GITHUB_REPO__.git`），日常在 `feature/description` 英文分支下编辑，不直接改 `dev` / `main`，改完 push 该分支。
...
- 生产环境最终部署目标（GitHub Pages / Cloudflare Pages / Vercel / 自托管）仍未决定，见 [待决策问题](open-decisions-zh.md)；本工作流只覆盖本地预览环节，与生产部署方式无关，二者可以独立演进。

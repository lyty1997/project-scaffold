...
占位。部署目标在[待决策问题](open-decisions-zh.md)里是未定项。
...
40 位小写提交 SHA。渲染器会补上符合仓库 Conventional Commit 规范
`<type>(<scope>): <English subject>` 的纯英文 Release PR 标题模式；当前渲染器固定使用
`chore` 类型。自定义模式也必须使用有效的 `chore` scope 形式、英文 subject，以及可解析的
`${version}` / `${branch}` 占位符。
...
| 层 | 位置 | 判定条件 | 表现 |
| --- | --- | --- | --- |
| 初始化出口 | `scripts/init.mjs` | 照抄行 141-154"跨机协同预览工作流"的可选章节范式，新增一段询问 | 选了就直接进搭建流程；不选则往[待决策问题](open-decisions-zh.md)的"部署目标"条目挂待办，并在"后续步骤"里打印 `npm run setup:cicd` |
...
| 2 | 就"探测不出来的事"逐项确认：构建命令、测试命令、部署目标、发布节奏；启用 Release 时再确认 release type、当前版本、版本源、历史起点、tag 与凭证模式 | 使用者答不上来的项先不生成对应 workflow，记进[待决策问题](open-decisions-zh.md) |
